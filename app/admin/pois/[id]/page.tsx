...
export default function EditPOIPage() {
    ...
    const fetchPOI = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // Convertir los datos del POI al formato del formulario
          setFormData({
            nameEs: poi.nameEs || '',
            nameEn: poi.nameEn || poi.nameEs || '',
            nameFr: poi.nameFr || poi.nameEs || '',
            nameDe: poi.nameDe || poi.nameEs || '',
            nameIt: poi.nameIt || poi.nameEs || '',
            descEs: poi.descEs || '',
            descEn: poi.descEn || poi.descEs || '',
            descFr: poi.descFr || poi.descEs || '',
            descDe: poi.descDe || poi.descEs || '',
            descIt: poi.descIt || poi.descEs || '',
            latitude: poi.lat?.toString() || '',
            longitude: poi.lng?.toString() || '',
            type: poi.category?.toLowerCase() || 'restaurant',
            address: poi.address || '',
            imageUrl: poi.images?.[0] || '',
            externalLink: poi.externalLink || '',
            points: poi.points?.toString() || '10',
            xpReward: poi.xpReward?.toString() || '50',
          });
        } else {
          alert('❌ Error al cargar el POI');
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-gray-600">Cargando POI...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold text-gray-800">✏️ Editar POI</h1>
        <p className="text-gray-600 mt-1">
          Modifica los datos del punto de interés
        </p>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        ...
        <ul className="text-sm text-blue-700 space-y-1">
          ...
          <li>• Puedes actualizar la ubicación si el POI se movió</li>
          <li>• Asegúrate de mantener una descripción clara</li>
        </ul>
      </div>
    </div>
  );
